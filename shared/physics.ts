...
export const stepVehicle = (vehicle: Vehicle, context: StepVehicleContext): void => {
  ...
  const steeringAuthority = 0.44 + (1 - speedSteerRatio) * 0.34 + (vehicle.isAirbraking ? 0.26 : 0)
  const handlingSteer = steer * clamp(steeringAuthority, 0.38, vehicle.isAirbraking ? 1.05 : 0.82)
  ...
  const yawScale = 1 - saturate(vehicle.telemetry.speedRatio) * (vehicle.isAirbraking ? 0.24 : 0.48)
  ...
  const speedGrip = 1 - saturate(vehicle.telemetry.speedRatio) * 0.24
  const movingGrip = profile.lateralGrip * Math.max(0.58, speedGrip + bankedControlRatio * BANKED_CONTROL.gripAssist)
